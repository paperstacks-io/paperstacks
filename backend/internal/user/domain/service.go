...
func NewUserService(repo Repository, authAPIURL string, httpClient *http.Client) *UserService {
	if httpClient == nil {
...
func (s *UserService) CreateFromToken(ctx context.Context, token string) (User, error) {
...
func (s *UserService) fetchUser(ctx context.Context, token string) (User, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, s.authAPIURL+"/me", nil)
	if err != nil {
		return User{}, fmt.Errorf("create session request: %w", err)
	}
	...
	if err != nil {
		return User{}, fmt.Errorf("send session request: %w", err)
	}
	defer res.Body.Close()

	if res.StatusCode != http.StatusOK {
		return User{}, fmt.Errorf("validate session: unexpected status %d", res.StatusCode)
	}

	var response meResponse
	if err := json.NewDecoder(res.Body).Decode(&response); err != nil {
		return User{}, fmt.Errorf("decode session response: %w", err)
	}
...
func (res *meResponse) ToUser() User {
	primaryEmail := ""
