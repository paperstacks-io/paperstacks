...
func (r *MemoryRepo) Create(paper domain_.Paper) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	if slices.ContainsFunc(r.data, func(p domain_.Paper) bool {
...
func (r *MemoryRepo) ReadAll() ([]domain_.Paper, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *MemoryRepo) Read(id string) (domain_.Paper, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()

	idx := slices.IndexFunc(r.data, func(p domain_.Paper) bool {
		return p.DOI == id
	})

	if idx == -1 {
		return domain_.Paper{}, ErrPaperNotFound
	}

	return r.data[idx], nil
}

func (r *MemoryRepo) Update(id string, paper domain_.Paper) error {
	r.mu.Lock()
	defer r.mu.Unlock()
