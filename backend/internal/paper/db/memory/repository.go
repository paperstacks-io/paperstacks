...
func (r *MemoryRepo) Create(_ context.Context, paper domain.Paper) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if slices.ContainsFunc(r.data, func(p domain.Paper) bool {
		...
		return errors.New("paper already exist")
	}
...
func (r *MemoryRepo) ReadAll(_ context.Context) ([]domain.Paper, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *MemoryRepo) Read(_ context.Context, id string) (domain.Paper, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	...
	if idx == -1 {
		return domain.Paper{}, errors.New("not found")
	}

	return r.data[idx], nil
}

func (r *MemoryRepo) Update(_ context.Context, id string, paper domain.Paper) error {
	r.mu.Lock()
	defer r.mu.Unlock()
